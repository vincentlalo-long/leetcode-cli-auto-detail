...
def main(config: Dict[str, Any]):
    """Run LeetCode testcases for a solution file."""
    print_command_banner("LeetCode Test")

    problem_num = styled_text_input("Enter problem number to test")
    ...
    if len(matches) == 1:
        ...
        selected = styled_select("Multiple matches found. Select file to test", choices)
    ...
    if not language_key:
        print_error("Unsupported file extension for testing.")
        return
    ...
    testcases = get_problem_testcases(slug)
    if not testcases:
        print_error("Could not fetch testcases from LeetCode.")
        return
    ...
    print_info("Running LeetCode testcases...")
